...
FROM ubuntu:23.10
WORKDIR ~
RUN apt-get update && apt-get install -y python3-venv pip wget
...
RUN wget https://pyropus.ca./software/memtester/old-versions/${MT_ARCHIVE} && \
    tar -xf ${MT_ARCHIVE} && cd ${MT_NAME} && make && cd ..
COPY memtester_parsing.py main.py ./
CMD python3 main.py --mt_args="100m 3" --mt_path=${MT_NAME}/memtester
